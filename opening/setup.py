...
def rev_comp(seq):
    return reduce(lambda a,b: a+b,[{'A':'T','T':'A','G':'C','C':'G'}[i] for i in seq[::-1]],'')
...
#note how the d1 loop is initially curved!

contour = np.linspace(0,1,10)
## Make a circle
x = np.sin(contour*2*np.pi)
y = np.zeros(len(contour))
z = np.cos(contour*2*np.pi) 
coords = np.array( (x,y,z) ).T
print(coords.shape)
print(coords) # Print first to illustrate data layout
dna_length = d1_loop_length*3.4
path_length = 2*np.pi          # half circle
coords = coords * (dna_length/path_length)
coords[:,2] += (d1_num_bp+d1_ssDNA+d1_toehold_length+d1_stem_length)*DNA_LENGTH

d1_loop = Section('s1_loop',None,None,num_bp = d1_loop_length,is_dsdna=False)

#D5 segment
Y = 100.
d5_num_bp = 28

TRANS_DOWN = np.array([0.,0.,-d5_num_bp*DNA_LENGTH])

d5 = Section('d5',np.array([0.,Y,0.]), np.array([0,Y,d5_num_bp*DNA_LENGTH]),num_bp = d5_num_bp)

d5_overhang = Section('d5_ssdna',np.array([0,Y,(d5_num_bp)*DNA_LENGTH]), np.array([0,Y,(d5_num_bp+d1_ssDNA)*DNA_LENGTH]),num_bp=d1_ssDNA,is_dsdna=False)

d5_toehold = Section('s5_toehold',np.array([0,Y,(d5_num_bp+d1_ssDNA)*DNA_LENGTH]),
                                  np.array([0,Y,(d5_num_bp+d1_ssDNA+d1_toehold_length)*DNA_LENGTH]),num_bp = d1_toehold_length, is_dsdna=False)

d5_stem = Section('s5_stem',np.array([0,Y,(d5_num_bp+d1_ssDNA+d1_toehold_length)*DNA_LENGTH]),
                            np.array([0,Y,(d5_num_bp+d1_ssDNA+d1_toehold_length+d1_stem_length)*DNA_LENGTH]),num_bp = d1_stem_length,is_dsdna=False)

for i in [d5,d5_overhang,d5_toehold,d5_stem]:
    i.start_position += TRANS_DOWN
    i.end_position += TRANS_DOWN
    

if d1_toehold_length != 0:
    all_segs = [d1,d1_overhang,d1_toehold,d1_stem,d1_loop,d5,d5_overhang,d5_toehold,d5_stem]
else:
    all_segs = [d1,d1_overhang,d1_stem,d1_loop,d5,d5_overhang,d5_stem]

for i in all_segs: i.create_helix()

d1_loop.dna.set_splines(contour,coords)
...
elif d1_toehold_length == 0:
    d1_stem.dna.connect_end3(d1_overhang.dna.start5)
d1_loop.dna.connect_start5(d1_stem.dna.start3)
d1_loop.dna.connect_end3(d1_stem.dna.start5)

d5.dna.connect_end3(d5_overhang.dna.start5)
if d1_toehold_length != 0:
    d5_toehold.dna.connect_start5(d5_overhang.dna.end3)
    d5_stem.dna.connect_start5(d5_toehold.dna.end3)
elif d1_toehold_length == 0:
    d5_stem.dna.connect_start5(d5_overhang.dna.end3)

segs_list = [x.dna for x in all_segs]

#breakpoint()

d1.dna.sequence = rev_comp('AGTAAGTGAATAATGAAGAAATAGATGTATAA')
d1_overhang.dna.sequence = 'T'*d1_ssDNA

if d1_toehold_length != 0:
    d1_toehold.dna.sequence = rev_comp("GGAAGGAGGAGAGGAGA"[:d1_toehold_length])
